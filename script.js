...
function checkForUpcomingGames(data) {
  ...
  if (notificationsDiv && notificationMessage) {
...
function attachEventListeners() {
  ...
  // Delegação de eventos para os botões das abas
  ...
  // Delegação de eventos para os botões de ação (Aplicar Filtros, Limpar Filtros, Pivot)
  document.addEventListener('click', (event) => {
    ...
    console.log(`[attachEventListeners] Clique no botão ${action} para tabId ${tabId}`);

    if (action === 'aplicarFiltros') {
      if (tabId === 'tab1') {
        displayTab1();
      } else if (tabId === 'tab2') {
        displayTab2();
      }
    } else if (action === 'limparFiltros') {
      clearFilters();
      if (tabId === 'tab1') {
        displayTab1();
      } else if (tabId === 'tab2') {
