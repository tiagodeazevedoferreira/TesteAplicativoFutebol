...
async function fetchSheetData() {
  ...
  try {
    ...
    if (errorMessage) {
      errorMessage.textContent = 'Erro ao carregar os dados da planilha. Verifique o console para detalhes.';
...
function addSortListeners(tabId) {
  ...
  sortableHeaders.forEach(header => {
    header.addEventListener('click', () => {
      console.log(`[addSortListeners] Clique no cabeçalho sortável para tabId ${tabId}, índice: ${header.dataset.index}`);
...
function updateBigNumbers(data, tabId) {
  ...
  const jogosElement = document.getElementById(`bigNumberJogos-${tabId}`);
  const golsElement = document.getElementById(`bigNumberGols-${tabId}`);
  const mediaElement = document.getElementById(`bigNumberMedia-${tabId}`);
  const assistenciasElement = document.getElementById(`bigNumberAssistencias-${tabId}`);
  const golACadaElement = document.getElementById(`bigNumberGolACada-${tabId}`);
  const vitoriasElement = document.getElementById(`bigNumberVitorias-${tabId}`);
  const empatesElement = document.getElementById(`bigNumberEmpates-${tabId}`);
  const derrotasElement = document.getElementById(`bigNumberDerrotas-${tabId}`);

  if (jogosElement) jogosElement.textContent = jogos;
  if (golsElement) golsElement.textContent = gols;
  if (mediaElement) mediaElement.textContent = mediaGols;
  if (assistenciasElement) assistenciasElement.textContent = assistencias;
  if (golACadaElement) golACadaElement.textContent = golACada;
  if (vitoriasElement) vitoriasElement.textContent = vitorias;
  if (empatesElement) empatesElement.textContent = empates;
  if (derrotasElement) derrotasElement.textContent = derrotas;
...
function updatePivotButtonContent(button, isPivot) {
  button.textContent = isPivot ? 'Normal' : 'Pivot';
  console.log(`[updatePivotButtonContent] Botão atualizado para: ${isPivot ? 'Normal' : 'Pivot'}`);
}

async function initializeData() {
  console.log('[initializeData] Inicializando dados');
  allData = await fetchSheetData();
  if (allData.length === 0) {
    console.error('[initializeData] Nenhum dado retornado da API');
    return false;
  }
  console.log('[initializeData] Dados carregados da API:', allData);
  ...
  console.log('[initializeData] Dados inicializados com sucesso');
  return true;
}

function attachEventListeners() {
  console.log('[attachEventListeners] Associando eventos aos botões');

  document.querySelectorAll('.tab-button').forEach(button => {
    button.addEventListener('click', () => {
      const tabId = button.id.split('-')[0];
      console.log(`[attachEventListeners] Clique no botão da aba ${tabId}`);
      showTab(tabId);
    });
  });

  document.querySelectorAll('button[id^="aplicarFiltros"]').forEach(button => {
    button.addEventListener('click', () => {
      const tabId = button.id.split('-')[1];
      console.log(`[attachEventListeners] Clique no botão Aplicar Filtros para tabId ${tabId}`);
      if (tabId === 'tab1') {
        displayTab1();
      } else if (tabId === 'tab2') {
        displayTab2();
  ...
  document.querySelectorAll('button[id^="limparFiltros"]').forEach(button => {
    button.addEventListener('click', () => {
      const tabId = button.id.split('-')[1];
      console.log(`[attachEventListeners] Clique no botão Limpar Filtros para tabId ${tabId}`);
      clearFilters();
      if (tabId === 'tab1') {
        displayTab1();
      } else if (tabId === 'tab2') {
  ...
  document.querySelectorAll('button[id^="pivotMode"]').forEach(button => {
    button.addEventListener('click', () => {
      const tabId = button.id.split('-')[1];
      console.log(`[attachEventListeners] Clique no botão Pivot para tabId ${tabId}`);
      isPivotMode[tabId] = !isPivotMode[tabId];
      console.log(`[attachEventListeners] Novo estado de isPivotMode[${tabId}]: ${isPivotMode[tabId]}`);
      updatePivotButtonContent(button, isPivotMode[tabId]);
      if (isPivotMode[tabId]) {
...
async function init() {
  ...
  if (!dataInitialized) {
    console.error('[init] Falha ao inicializar os dados');
    return;
  }
